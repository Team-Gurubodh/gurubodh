"""Repeat S2 contract cases through a real non-editable installation."""
...
def main():
    ...
    for kind in ("subject-manifest", "locale-definition"):
    ...
    test_path = Path(__file__).with_name("test_subject_locale_contracts.py")
    spec = importlib.util.spec_from_file_location("installed_s2_cases", test_path)
    ...
    def run_cases():
        ...
        suite = unittest.defaultTestLoader.loadTestsFromTestCase(module.SubjectLocaleContractTests)
    ...
    hidden = jobs.with_name("jobs-hidden-for-s2-check")
    jobs.rename(hidden)
    try:
        run_cases()
    finally:
        ...
        validation.schema_path.cache_clear()
        validation._validator.cache_clear()
    print("Installed S2 contracts passed with job schemas present and absent.")
