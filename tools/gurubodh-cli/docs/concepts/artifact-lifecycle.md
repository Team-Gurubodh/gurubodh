...
The versioned canonical `.txt` under `chapters/text_and_metadata/` is the proofread source of truth. Its metadata includes an exact artifact-byte SHA-256 and a deterministic `content_identity`. The latter normalizes to NFC Unicode, LF line endings, no trailing spaces/tabs per line, and no outer Unicode whitespace. Text edits create a new provenance key; reordering unchanged text does not.
