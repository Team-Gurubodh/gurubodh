...
def resolve_project_context(project_root=None):
    if project_root:
        ...
        env_root = os.environ.get("GURUBODH_UTILS_ROOT")
        if env_root:
            root = Path(env_root).expanduser().resolve()
            if not _looks_like_project_root(root):
                raise SystemExit(f"GURUBODH_UTILS_ROOT is not a Gurubodh CMS project root: {root}")
        else:
            root = _find_project_root(Path.cwd())
            if root is None:
                raise SystemExit(
                    "Could not find project root. Run from the project tree, set "
                    "GURUBODH_UTILS_ROOT, or pass --project-root."
                )
