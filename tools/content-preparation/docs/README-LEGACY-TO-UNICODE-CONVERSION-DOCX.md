...
`gurubodh-utils ...` console command can be used from any directory.

Root detection order:

1. `--project-root`, if supplied.
2. `GURUBODH_UTILS_ROOT`, if set.
...
"processing": {
  "pipeline": "unicode-docx-ingest",
  "entry_point": "python3 -m gurubodh_utils run"
}
```
...
"processing": {
  "pipeline": "unicode-docx-ingest",
  "entry_point": "python3 -m gurubodh_utils unicode-ingest"
}
```
